...
var native_lib_path = path.join(__dirname, '../build/Release/pel_printer.node');
var printer_helper;

if (fs.existsSync(native_lib_path)) {
  printer_helper = require(native_lib_path);
} else {
  printer_helper = require('./pel_printer-' + process.platform + '-' + process.arch + '.node');
}
...
function printDirect(parameters) {
  var data = parameters,
    printer, docname, type, options, success, error;
